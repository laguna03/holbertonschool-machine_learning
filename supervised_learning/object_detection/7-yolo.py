...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        This function draws rectangles around detected objects in an
        image, annotates them with the
        class names and the confidence scores, and displays the image.
        The user can save the annotated
        image by pressing 's'.

        Parameters:
        - image (np.ndarray): The image on which to draw the boxes.
        - boxes (np.ndarray): Array of bounding box coordinates
        (x1, y1, x2, y2)
        for each detection.
        - box_classes (np.ndarray): Array of class indices for each
        detected object.
        - box_scores (np.ndarray): Array of scores for each detection.
        - file_name (str): The name of the file where the image with
        drawn boxes
        will be saved if 's' is pressed.

        Note:
        - The function assumes the use of OpenCV for image manipulation
        and display.
